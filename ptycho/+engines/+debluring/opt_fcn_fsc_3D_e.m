% integrated_resolution = fourier_shell_corr_3D_2e(image1,image2,kernel,param)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function integrated_resolution= opt_fcn_fsc_3D_e(img1,image2,kernel,param)
import math.isint
import utils.*
...
% img2_real = real(image2);
% img2_imag = imag(image2);
% deconv_img2_imag = deconvlucy(img2_imag, kernel,param.lucy_iters,param.damping_threshold);
% img2 = img2_real + 1j * deconv_img2_imag;
img2 = ifft2(fft2(image2) .* conj(fft_kernel([size(img1,1),size(img1,2)],kernel) ./ abs(fft_kernel([size(img1,1),size(img1,2)],kernel)+0.0001)));
